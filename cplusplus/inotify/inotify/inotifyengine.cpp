...
//            inotify_rm_watch(d->m_inotifyFd, event.wd);
            QString removedFile =  d->m_paths.value(event.wd);
        } else if (event.mask & IN_MASK_ADD == 0) {
            QString addFile = d->m_paths.value(event.wd);
        }
    }
}
...
bool InotifyEngine::watchDirectory(QDir &dir)
{
    uint32_t dirMask  = 0 | IN_ATTRIB | IN_MOVE | IN_CREATE | IN_DELETE
                        | IN_DELETE_SELF | IN_MOVE_SELF | IN_MASK_ADD;
